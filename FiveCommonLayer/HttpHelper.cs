        ...
        public static string DownloadHtml(string url, System.Text.Encoding encode)
        {
            ...
            try
            {
                ...
                using (HttpWebResponse response = request.GetResponse() as HttpWebResponse)//发起请求
                {
                    ...
                    {
                        MyLog.OutputAndSaveTxt((string.Format("抓取{0}地址返回失败,response.StatusCode为{1}", url, response.StatusCode)));
                    }
                    else
                    {
                        try
                        ...
                        {
                            MyLog.OutputAndSaveTxt(string.Format("DownloadHtml抓取{0}保存失败。Ex:{1}", url, ex));
                            html = null;
                        }
                    }
                }
            ...
            catch (System.Net.WebException ex)
            {
                ...
                {
                    MyLog.OutputAndSaveTxt(string.Format("远程服务器返回错误: (306)。Ex:{1}", ex));
                    return null;
                }
            }
            catch (Exception ex)
            {
                MyLog.OutputAndSaveTxt(string.Format("DownloadHtml抓取{0}出现异常。Ex:{1}", url, ex));
                html = null;
            }
            return html;
        }
